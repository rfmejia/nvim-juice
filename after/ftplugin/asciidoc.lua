...
util["set-keys"]({{"n", "<localleader>d", ":r!date '+\\%a, \\%d \\%b \\%Y' | xargs -0 printf '\\n== \\%s\\n\\n'<cr>k", {desc = "insert current date as an h2 header", buffer = vim.api.nvim_get_current_buf(), noremap = true, silent = true}}, {"n", "<localleader>t", ":r!date '+\\%H:\\%M' | xargs -0 printf '=== \\%s ' | tr -d '\\n'<cr>A", {desc = "insert current time as an h3 header", buffer = vim.api.nvim_get_current_buf(), noremap = true, silent = true}}})
local function preview_in_browser(_in, out, browser_cmd)
  _G.assert((nil ~= browser_cmd), "Missing argument browser-cmd on /home/rfmejia/.config/nvim/fnl/after/ftplugin/asciidoc.fnl:27")
  _G.assert((nil ~= out), "Missing argument out on /home/rfmejia/.config/nvim/fnl/after/ftplugin/asciidoc.fnl:27")
  _G.assert((nil ~= _in), "Missing argument in on /home/rfmejia/.config/nvim/fnl/after/ftplugin/asciidoc.fnl:27")
...
if vim.env.BROWSER then
  ...
  return vim.keymap.set("n", "<localleader>p", _7_, {desc = "convert to HTML and show preview in browser", buffer = vim.api.nvim_get_current_buf(), noremap = true, silent = true})
else
  return nil
end
