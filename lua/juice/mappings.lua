...
general = {{"n", "Y", "y$", {desc = "yank until the end of the line"}}, {"n", "<leader>;", ":<C-r>\"", {desc = "paste register 0 contents in command mode"}}, {"n", "<leader>w", vim.cmd.w, {desc = "write buffer", silent = true}}, {"n", "<leader>r", vim.cmd.registers, {desc = "list registers"}}, {"n", "<leader>f", ":find<space>", {desc = "pre-fill find command"}}, {"n", "<F2>", "let @+ = getreg('%')", {desc = "copy current file path to clipboard"}}, {"n", "<F5>", vim.cmd.make, {desc = "trigger `make` in shell"}}, {"n", "<leader>n", _2_, {desc = "toggle number and relativenumber options"}}, {"n", "<leader>ol", ":Lazy<cr>", {desc = "open lazy.nvim", silent = true}}}
...
do
  ...
  marks = core.concat({{"n", "''", _5_, {desc = "list quick marks (ARST and zxcd)"}}}, core.map(_6_, marks0), core.map(_7_, marks0))
end
...
journal_launchers = {{"n", "<leader>oj", _8_, {desc = "open journal in a new tab", silent = true}}, {"n", "<leader>ov", _9_, {desc = "open vim notes in a new tab", silent = true}}}
...
oil_maps = {{"n", "<leader>e", _10_, {desc = "[oil] explore files in current file's path", silent = true}}}
...
telescope_maps = {{"n", "<leader>p", _11_, {desc = "[telescope] oldfiles"}}, {"n", "<leader>g", _12_, {desc = "[telescope] (g)it files"}}, {"n", "<leader>k", _13_, {desc = "[telescope] (k)eymaps"}}}
local gitsigns_maps
do
  local nav
  local function _14_()
