...
general = {{"n", "Y", "y$", {desc = "yank until the end of the line"}}, {"n", "<leader>w", vim.cmd.w, {desc = "write buffer", silent = true}}, {"n", "<leader>r", vim.cmd.registers, {desc = "list registers"}}, {"n", "<F2>", "let @+ = getreg('%')", {desc = "copy current file path to clipboard"}}, {"n", "<F5>", vim.cmd.make, {desc = "trigger `make` in shell"}}, {"n", "]<space>", "o<esc>k"}, {"n", "[<space>", "O<esc>j"}, {"n", "<leader>n", _2_, {desc = "toggle number and relativenumber options"}}, {"n", "<leader>ol", ":Lazy<cr>", {desc = "open lazy.nvim", silent = true}}}
local filters
do
  local _repeat
...
journal_launchers = {{"n", "<leader>oj", _10_, {desc = "open journal in a new tab", silent = true}}, {"n", "<leader>ov", _11_, {desc = "open vim notes in a new tab", silent = true}}}
...
oil_maps = {{"n", "<leader>e", _12_, {desc = "[oil] explore files in current file's path", silent = true}}}
local gitsigns_maps
do
  local nav
  local function _13_()
  ...
  nav = {{"n", "]g", _13_, {desc = "[gitsigns] jump to next git hunk"}}, {"n", "[g", _14_, {desc = "[gitsigns] jump to previous git hunk"}}}
  local staging
  local function _15_()
  ...
  staging = {{"n", "<localleader>gs", _15_, {desc = "[gitsigns] (g)it (s)tage hunk"}}, {"n", "<localleader>gu", _16_, {desc = "[gitsigns] (g)it (u)ndo staged hunk"}}, {"n", "<localleader>gr", _17_, {desc = "(g)it (r)eset hunk"}}, {"n", "<localleader>gS", _18_, {desc = "[gitsigns] (g)it (S)tage buffer"}}, {"n", "<localleader>gR", _19_, {desc = "[gitsigns] (g)it (R)eset buffer"}}, {"v", "<localleader>gs", _20_, {desc = "[gitsigns] (g)it (s)tage hunk"}}, {"v", "<localleader>gr", _22_, {desc = "[gitsigns] (g)it (r)eset hunk"}}}
  local blame
  local function _23_()
  ...
  blame = {{"n", "<localleader>gb", _23_, {desc = "[gitsigns] (g)it show line (b)lame"}}, {"n", "<localleader>gB", _24_, {desc = "[gitsigns] (g)it toggle current line (B)lame"}}}
  local view
  local function _25_()
  ...
  view = {{"n", "<localleader>gt", _25_, {desc = "[gitsigns] toggle sign visibility"}}, {"n", "<localleader>gp", _26_, {desc = "[gitsigns] (g)it (p)review hunk"}}, {"n", "<localleader>gd", _27_, {desc = "[gitsigns] (g)it show (d)iff"}}, {"n", "<localleader>gD", _28_, {desc = "[gitsigns] (g)it toggle (D)eleted hunks"}}}
  local list
  local function _29_()
  ...
  list = {{"n", "<localleader>gl", _29_, {desc = "[gitsigns] show buffer (g)it hunks in (l)oclist"}}, {"n", "<localleader>gc", _30_, {desc = "[gitsigns] show all (g)it hunks in qui(c)kfix list"}}}
