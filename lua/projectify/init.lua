...
local function load_hashes(path)
  if (nil == path) then
    _G.error("Missing argument path on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:14", 2)
  else
  end
...
local function save_hashes(path, obj)
  if (nil == obj) then
    _G.error("Missing argument obj on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:17", 2)
  else
  end
  if (nil == path) then
    _G.error("Missing argument path on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:17", 2)
  else
  end
  local now = os.time()
  obj["updated"] = now
...
local function compute_hash(input_string)
  if (nil == input_string) then
    _G.error("Missing argument input-string on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:26", 2)
  else
  end
...
local function allow_project(_12_, allowed)
  local path = _12_.path
  local source = _12_.source
  if (nil == allowed) then
    _G.error("Missing argument allowed on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:55", 2)
  else
  end
  if (nil == source) then
    _G.error("Missing argument source on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:55", 2)
  else
  end
  if (nil == path) then
    _G.error("Missing argument path on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:55", 2)
  else
  end
...
local function ask_allow_project(project, _3fprompt)
  if (nil == project) then
    _G.error("Missing argument project on /home/rfmejia/.config/nvim/fnl/projectify/init.fnl:64", 2)
  else
  end
  local prompt
  local _17_
  ...
  return vim.ui.input({prompt = prompt}, _19_)
end
...
local function setup()
  ...
  vim.api.nvim_create_user_command("ProjectifyAllow", _22_, {desc = "[projectify] Allow and load project file"})
  local function _23_()
    allow_project(read_local_project_file(), false)
    return vim.notify("Project disallowed", vim.log.levels.INFO)
  end
  return vim.api.nvim_create_user_command("ProjectifyDisallow", _23_, {desc = "[projectify] Disallow project file"})
end
