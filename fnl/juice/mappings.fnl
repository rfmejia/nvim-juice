...
(comment "Disable default mappings to train optimal alternatives")
(u.nmap "{" (fn [] (print "Use C-u or relative jumps instead"))
        [:noremap :silent])

(u.nmap "}" (fn [] (print "Use C-d or relative jumps instead"))
        [:noremap :silent])
...
(u.nmap :<leader>mc ":delmarks ARST<cr>:echo 'Cleared file marks'<cr>"
        [:noremap])
...
(when (u.exists? :$TMUX)
  ...
  (when (u.executable? :sbtn)
    (u.nmap :<leader>os ":!tmux split-window -v -l 30\\% sbtn<cr><cr>"
            [:noremap :silent]))
...
(when (u.exists? :$JOURNAL)
  (u.nmap :<leader>oj (fn [] (vim.cmd (.. ":$tabnew" :$JOURNAL/journal.adoc)))
          [:noremap :silent])
  (u.nmap :<leader>ov (fn [] (vim.cmd (.. ":$tabnew" :$JOURNAL/vim/vim.md)))
          [:noremap :silent]))
...
(let [gitsigns (require :gitsigns)]
  ...
  (u.nmap :<localleader>hS (fn [] (gitsigns.blame_line {:full true}))
          [:noremap])
