...
(fn setup []
  (let [scalametals (autoload :juice.lsp.scalametals)
    ...
    (lspconfig.ts_ls.setup {:on_attach set-buffer-opts})
    (lspconfig.jdtls.setup {:on_attach set-buffer-opts})
    (lspconfig.clojure_lsp.setup {:on_attach set-buffer-opts})
    (lspconfig.gopls.setup {:on_attach set-buffer-opts : go-settings})))
