...
(let [u (autoload :juice.util)]
  ...
  (set vim.opt.spelllang :en_us) ; TODO fix and verify this works
  ...
  (u.nmap :<localleader>m
          (u.lua-cmd "require('juice.filetypes.markdown')['insert-yaml-metadata']()")
          [:noremap :silent])
  (u.nmap :<localleader>v
          (u.lua-cmd "require('juice.filetypes.markdown')['render-markdown-to-html']()")
          [:noremap :silent])
  (u.nmap :<localleader>d
          ":r!date '+\\%a, \\%d \\%b \\%Y' | xargs -0 printf '----\\n\\n\\%s\\n'<cr>"
          [:noremap :silent])
  (u.nmap :<localleader>t
